    ...
    <nav class="flex items-center justify-between text-black bg-navbar shadow-xs h-16">
        <div class="flex items-center flex-no-shrink">
            <a href="{{ url('/') }}" class="flex items-center flex-no-shrink text-black mx-4">
                ...
                @if($admin_favicon == '')
                    <img src="{{ asset('images/icon.png') }}" alt="{{ setting('admin.title') }}" width="40px">
                @else
                    <img src="{{ Voyager::image($admin_favicon) }}" alt="{{ setting('admin.title') }}" width="40px">
                @endif
                {{-- @include("larecipe::partials.logo") --}}

                <p class="inline-block font-semibold mx-1 text-grey-dark" style="margin-left: 10px">
                    {{ setting('admin.title') }}
                </p>
            </a>
