...
@if(auth()->user()->hasPermission('browse_people_exts'))
                                                    ...
                                                    <td class="text-right" style="white-space: nowrap;">
                                                        ...
                                                        @else
                                                            <a href="" data-toggle="modal" data-target="#modalActivo" data-id="{{$item->id}}" class="btn btn-sm btn-success">
                                                                <i class="voyager-check"></i> Habilitar
                                                            </a>
                                                        @endif
                                                    </td>
                                                </tr>
        ...
        <div class="modal fade" role="dialog" id="modalEditar">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    {!! Form::open(['route' => ['people_exts.update', 0], 'method' => 'PUT', 'class' => 'was-validated', 'id' => 'formEditar'])!!}
                    <!-- Modal body -->
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select name="person_id" id="edit_person_id" class="form-control select2" required>
                                        <option value="">Seleccione una opcion</option>
                                        @foreach($people as $data)
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select name="direccion_id" id="edit_direccion_id" class="form-control select2" required>
                                        <option value="">Seleccione una opcion</option>
                                        @foreach($direcciones as $data)
                                            <option value="{{$data->id}}">{{$data->nombre}}</option>
                                        @endforeach
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <select name="unidad_id" id="edit_unidad_id" class="form-control select2" required>
                                        <option value="">Seleccione una opcion</option>
                                        @foreach($unidades as $data)
                                            <option value="{{$data->id}}">{{$data->nombre}}</option>
                                        @endforeach
    ...
    @section('javascript')
        ...
        <script>
            ...
            var baseEditAction = $('#formEditar').attr('action');
            $('#modalEditar').on('show.bs.modal', function (event) {
                var button = $(event.relatedTarget)
                var item = button.data('item')
                var modal = $(this)
                var form = modal.find('form')
                form.attr('action', baseEditAction.replace('/0', '/' + item.id))
                modal.find('#edit_id').val(item.id)
                modal.find('#edit_person_id').val(item.person_id).trigger('change')
                modal.find('#edit_direccion_id').val(item.direccion_id).trigger('change')
                modal.find('#edit_unidad_id').val(item.unidad_id).trigger('change')
                modal.find('#edit_observacion').val(item.observacion)
                modal.find('#edit_cargo').val(item.cargo)
            });
            $('#modalActivo').on('show.bs.modal', function (event) {
                var id = $(event.relatedTarget).data('id')
                $(this).find('#activo_id').val(id)
            });
